...
public sealed class UpdateClientStageCommandHandler(IClientRepository repository, ISettingsReferenceDataClient settingsReferenceData)
    : IRequestHandler<UpdateClientStageCommand, ClientDto?>
{
    public async Task<ClientDto?> Handle(UpdateClientStageCommand request, CancellationToken cancellationToken)
    {
        var entity = await repository.GetByIdAsync(request.ClientId, cancellationToken);
        if (entity == null) return null;

        entity.ClientStatusId = request.ClientStatusId;
