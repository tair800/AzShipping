...
namespace Carrier.Infrastructure.Services;
    ...
    ILogger<ActionLogClient> logger) : IActionLogClient
{
    public async Task LogAsync(string action, string data, Guid? employeeId = null, string? employeeName = null, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var sessionId = ctx?.Request?.Headers.TryGetValue("X-Session-Id", out var sidVal) == true && !string.IsNullOrWhiteSpace(sidVal)
                ? sidVal.ToString().Trim()
                : null;
            if (!string.IsNullOrEmpty(sessionId))
                Console.WriteLine("[Carrier ActionLog] X-Session-Id present: {0} for action: {1}", sessionId, action);
            else
                Console.WriteLine("[Carrier ActionLog] WARNING: No X-Session-Id in request for action: {0} -> will NOT appear in Session Log", action);
