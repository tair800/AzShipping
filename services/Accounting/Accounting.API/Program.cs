...
using Accounting.Application.Extensions;
...
builder.Services.AddControllers(o => o.Filters.Add<DocumentsErpPermissionFilter>())
    .AddJsonOptions(o =>
...
// Ensure AccountingDb exists
var conn = builder.Configuration.GetConnectionString("DefaultConnection");
if (!string.IsNullOrEmpty(conn))
{
    var dbName = "AccountingDb";
...
using (var scope = app.Services.CreateScope())
{
    ...
    var log = scope.ServiceProvider.GetService<ILoggerFactory>()?.CreateLogger("AccountingDbSeeder");
    await AccountingDbSeeder.SeedAsync(db, log);
    await AccountingDbSeeder.SeedInvoiceLookupsAsync(db, log);
    await AccountingDbSeeder.SeedSampleOperationInvoicesAsync(db, log);
    await AccountingDbSeeder.SeedSampleOperationActsAsync(db, log);
}

app.UseCustomSwagger("Accounting");
...
app.MapGet("/health", () => Results.Ok(new { service = "Accounting.API", status = "ok" }))
    .AllowAnonymous()
    .ExcludeFromDescription();
